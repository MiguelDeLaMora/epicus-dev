    ...
    <header className="fixed top-0 left-0 right-0 z-50 h-[68px] bg-white/95 backdrop-blur-[16px] border-b border-black/[0.08] flex items-center">
      <div className="w-full px-[4%] flex items-center justify-between">
        ...
        <Link href="/" className="hover:opacity-80 transition-opacity duration-200">
          <Image
            src="/logos/epicus-black-logo.webp"
            alt="EPICUS"
            width={120}
            height={32}
        ...
        >
          <span className={cn("block h-px bg-current transition-all duration-200 origin-center", open ? "w-6 rotate-45 translate-y-[6px]" : "w-6")} />
          <span className={cn("block h-px bg-current transition-all duration-200", open ? "opacity-0 w-0" : "w-4")} />
          <span className={cn("block h-px bg-current transition-all duration-200 origin-center", open ? "w-6 -rotate-45 -translate-y-[6px]" : "w-6")} />
        </button>
      </div>
      ...
      {/* Menú mobile */}
      {open && (
        <div className="absolute top-[68px] left-0 right-0 bg-white border-b border-black/[0.08] flex flex-col px-[4%] py-6 gap-5 md:hidden">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
