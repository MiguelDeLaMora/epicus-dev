...
function TipologiaCard({ icon, name, desc, zones }: TipologiaProps) {
  return (
    <div className="bg-white p-8 border border-black/[0.06]">
      ...
      <div className="flex flex-wrap gap-2">
        {zones.map((z) => (
          <span
            key={z}
            className="text-[11px] text-ink-soft tracking-[0.04em] px-3 py-1 bg-neutral-100 rounded-[2px]"
          >
            {z}
          </span>
        ))}
...
const tipologias: TipologiaProps[] = [
  {
    icon: <IconTerreno />,
    name: "Terrenos Residenciales",
    desc: "Para construir tu hogar en las zonas de mayor proyección y plusvalía en Monterrey y área metropolitana.",
    zones: ["Carr. Nacional", "Santiago", "García"],
  },
  {
    icon: <IconDepto />,
    name: "Departamentos",
    desc: "Verticales residenciales en zonas clave de la ciudad, desde opciones de entrada hasta segmento premium.",
    zones: ["Valle Oriente", "San Jerónimo", "Santa María"],
  },
  {
    icon: <IconIndustrial />,
    name: "Industrial",
    desc: "Terrenos y bodegas industriales en los corredores logísticos más activos de Nuevo León. Inversión con flujo.",
    zones: ["Santa Catarina", "Salinas", "García"],
  },
  {
    icon: <IconCasa />,
    name: "Casas",
    desc: "Proyectos de vivienda horizontal en fraccionamientos cerrados con amenidades y ubicación estratégica.",
    zones: ["Apodaca", "Monterrey", "Guadalupe"],
  },
];
  ...
  return (
    <Section className="bg-cream">
      ...
      <div className="flex items-start gap-4 mb-14 p-6 bg-white border border-black/[0.06]">
        <svg viewBox="0 0 24 24" className="w-5 h-5 mt-[2px] shrink-0 stroke-blue fill-none stroke-[1.5] [stroke-linecap:round] [stroke-linejoin:round]">
        ...
        <p className="text-[14px] font-light text-ink-mid leading-[1.8]">
          ...
          Trabajamos exclusivamente con desarrolladores de trayectoria comprobada,
          proyectos con permisos en regla y estructuras legales transparentes.
          Antes de presentarte una opción, nosotros ya hicimos la tarea.
        </p>
      </div>
      ...
      <div className="mt-12 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 p-8 bg-white border border-black/[0.06]">
        <div>
          <h3 className="font-display text-h4 text-ink mb-2">
            ¿Listo para ver opciones reales?
          </h3>
          <p className="text-[14px] font-light text-ink-mid">
            En 30 minutos te mostramos las mejores alternativas del mercado,
            filtradas para tu perfil y objetivos.
          </p>
        </div>
        <Link
          href="/contacto"
